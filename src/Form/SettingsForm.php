...
final class SettingsForm extends ConfigFormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state): array {
    ...
    $form['trusted_hosts'] = [
      ...
      '#description' => $this->t('Enter one host name per line, without a path (for example, media.example.org). Iframes from these hosts and their subdomains are loaded without asking for consent.'),
    ...
    $form['fallback_image_fid'] = [
      ...
      '#description' => $this->t('Used when the external service does not expose an oEmbed thumbnail. Leave empty to use the image supplied with the module.'),
  ...
  public function validateForm(array &$form, FormStateInterface $form_state): void {
    ...
    $hosts = preg_split('/\R/', (string) $form_state->getValue('trusted_hosts'), -1, PREG_SPLIT_NO_EMPTY) ?: [];
    $normalized = [];
    foreach ($hosts as $host) {
      $host = strtolower(rtrim(trim($host), '.'));
      if (!preg_match('/^(?:[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?\.)*[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?$/', $host)) {
        $form_state->setErrorByName('trusted_hosts', $this->t('%host is not a valid host name.', ['%host' => $host]));
      }
      else {
  ...
  public function submitForm(array &$form, FormStateInterface $form_state): void {
    ...
    if ($new_fid !== $old_fid) {
      if ($old_fid > 0 && ($old_file = $this->entityTypeManager->getStorage('file')->load($old_fid))) {
        $this->fileUsage->delete($old_file, 'rouen_iframe_consent', 'fallback', 'global');
        if ($this->fileUsage->listUsage($old_file) === []) {
          $old_file->delete();
        }
      }
      if ($new_fid > 0 && ($new_file = $this->entityTypeManager->getStorage('file')->load($new_fid))) {
        $new_file->setPermanent();
        $new_file->save();
        $this->fileUsage->add($new_file, 'rouen_iframe_consent', 'fallback', 'global');
      }
    }
