...
final class ThumbnailManager {
   ...
   */
  public function ensureThumbnail(EntityInterface $entity, string $fieldName, int $delta, ParsedIframe $iframe): ?string {
    if (!$entity instanceof FieldableEntityInterface || $entity->isNew() || $entity->uuid() === NULL) {
      return NULL;
    }
    ...
    $record = $this->database->select(self::TABLE, 't')
      ->fields('t')
    ...
    if ($record !== FALSE && hash_equals($record->source_hash, $iframe->getSourceHash())) {
      ...
      $this->database->update(self::TABLE)
        ...
        ->execute();
      $this->queueFactory->get(self::QUEUE)->createItem(['record_id' => (int) $record->id]);
      return NULL;
    }
    ...
    if ($record !== FALSE) {
      ...
      try {
        $record_id = (int) $this->database->insert(self::TABLE)
          ->fields($keys + [
            'entity_id' => (string) $entity->id(),
            'source_hash' => $iframe->getSourceHash(),
            'iframe_url' => $iframe->thumbnailLookupUrl,
    ...
    $this->queueFactory->get(self::QUEUE)->createItem(['record_id' => $record_id]);
    return NULL;
  }
  ...
  public function syncEntity(EntityInterface $entity): void {
    ...
    $existing = $this->database->select(self::TABLE, 't')
      ->fields('t')
      ...
      ->fetchAllAssoc('id');
    $retained_ids = [];
    $trusted_hosts = $this->configFactory->get('rouen_iframe_consent.settings')->get('trusted_hosts') ?: [];

    foreach ($entity->getFieldDefinitions() as $field_name => $definition) {
      if (!in_array($definition->getType(), ['text', 'text_long', 'text_with_summary'], TRUE)
      ...
      foreach ($entity->get($field_name) as $delta => $item) {
        ...
        if ($parsed === NULL || $this->iframeParser->isTrustedHost($parsed->host, $trusted_hosts)) {
          continue;
        }
        $this->ensureThumbnail($entity, $field_name, (int) $delta, $parsed);
        foreach ($existing as $id => $record) {
          if ($record->field_name === $field_name && (int) $record->delta === (int) $delta) {
  ...
  public function deleteForEntity(EntityInterface $entity): void {
    ...
    $records = $this->database->select(self::TABLE, 't')
      ->fields('t')
      ->condition('entity_type', $entity->getEntityTypeId())
      ->condition('entity_uuid', $entity->uuid())
      ->execute()
  ...
  public function processThumbnail(int $recordId): void {
    ...
    try {
      ...
      if (!$this->isSafeRemoteImageUrl($thumbnail_url_string)) {
        throw new \RuntimeException('The oEmbed thumbnail URL is not safe to retrieve.');
      }
      ...
      if ($content_length > self::MAX_DOWNLOAD_BYTES) {
        throw new \RuntimeException('The oEmbed thumbnail exceeds the size limit.');
      }
      $mime_type = strtolower(trim(explode(';', $response->getHeaderLine('Content-Type'))[0]));
      $extensions = [
        'image/jpeg' => 'jpg',
        'image/png' => 'png',
        'image/gif' => 'gif',
      ...
      if (!isset($extensions[$mime_type])) {
        throw new \RuntimeException('The oEmbed thumbnail has an unsupported media type.');
      }

      $data = $response->getBody()->read(self::MAX_DOWNLOAD_BYTES + 1);
      if ($data === '' || strlen($data) > self::MAX_DOWNLOAD_BYTES) {
        throw new \RuntimeException('The oEmbed thumbnail is empty or exceeds the size limit.');
      }

      $directory = 'public://rouen_iframe_consent/thumbnails';
      if (!$this->fileSystem->prepareDirectory($directory, FileSystemInterface::CREATE_DIRECTORY | FileSystemInterface::MODIFY_PERMISSIONS)) {
        throw new \RuntimeException('The thumbnail directory could not be prepared.');
      }
      $image_info = @getimagesizefromstring($data);
      if ($image_info === FALSE || ($image_info['mime'] ?? NULL) !== $mime_type) {
        throw new \RuntimeException('The downloaded data is not a valid image.');
      }

      $destination = $directory . '/' . $record->source_hash . '-' . $recordId . '.' . $extensions[$mime_type];
      ...
      $this->logger->warning('Thumbnail retrieval failed for @url: @message', [
        '@url' => $record->iframe_url,
        '@message' => $exception->getMessage(),
      ]);
      if ($attempts < 3) {
        throw new RequeueException('Thumbnail retrieval will be retried.', 0, $exception);
      }
      $this->markFailed($recordId, $record->source_hash);
    }
  }
   ...
   */
  private function fieldUsesFormatter(FieldableEntityInterface $entity, string $fieldName): bool {
    $displays = $this->entityDisplayRepository->getViewDisplays($entity->getEntityTypeId(), $entity->bundle());
  ...
  private function invalidateRecord(object $record): void {
    $definition = $this->entityTypeManager->getDefinition($record->entity_type, FALSE);
    if ($definition !== NULL) {
      $this->cacheTagsInvalidator->invalidateTags([$record->entity_type . ':' . $record->entity_id]);
    }
  }
